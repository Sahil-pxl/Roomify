...
router.post('/api/projects/save', async ({request,user})=>{
    try {
        ...
        if(!project?.id ||projeect?.sourceImage) return jsonError(400,' Project not found');
        ...
        const key = `${PROJECT_PREFIX}${userPuter.id}`;
        await userPuter.kv.set(key,payload);

        return {saved : true, id:project.id, project: payload}

    } catch (e) {
        return jsonError(500,'failed to save project',{meessage: e.meessage || 'unknown error'})
    }
})
